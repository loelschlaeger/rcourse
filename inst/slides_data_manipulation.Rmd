...
We can make the last chunk more compact using the pipe operator `%>%` (contained in {dplyr}):

```{r}
metrics %>%
  select(symbol, period, cr, liabilities) %>%
  mutate(cash = cr * liabilities)
```

The output of `select()` (which is a data set) is the first argument of `mutate()`.
...
Again, we can make the last chunk more compact using the pipe operator `%>%`:
