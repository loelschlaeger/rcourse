...
What is the difference between `&`, `|` and `xor()`? Try to find out yourself with the following code:
...
quiz(
  ...
  question("Which rows does `filter(stocks, hq == \"Texas\", hq == \"New York\")` print?",
    answer("produces an error"),
    answer("all rows in `stocks`"),
    answer("all rows with either `hq` either equal to Texas or New York"),
...
```{r select-solution}
# only some options...
select(metrics, symbol, period, rd)
select(metrics, 1, 2, 5)
select(metrics, 1:5) %>%
  select(!3:4)
```
...
```{r rd-solution}
rename(metrics, research_investment = rd)
```


### Create columns

Use the columns `eps` and `ni` from `metrics` to create a column `shares` with the company's number of shares.
...
```{r shares-solution}
select(metrics, ni, eps) %>% 
  mutate(shares = ni/eps)
```
...
mutate(metrics, shares = ni/eps)
transmute(metrics, shares = ni/eps)
```
...
quiz(
  ...
  question("What will the `symbol` column in `mutate(metrics, symbol = NULL)` look like?",
...
From the help page we know that `prices` contains closing prices and trading volumes at the New York Stock Exchange from 2012 to 2016. How many records per stock does the data set have? 
